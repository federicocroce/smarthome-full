...
def sensor_init(params):
    ...
    def handle_motion(pin):
        ...
        print('client', client)
        if client:
            client.publish('sendTelegramMessage', 'sendTelegramMessage Sensor ' + name)
        ...
        def reset_alarm_count(timer):
            ...
            client.publish('sendTelegramMessage', ujson.dumps({'message': "Se reinicia el contador", 'name': name}))
            count = 0
        ...
        def deactive_alarm(timer):
            ...
            client.publish('sendTelegramMessage', ujson.dumps({'message': "se desactivó la alarma porque transcurrieron", 'name': name}))
            alarm_is_active = False
            ALERT_STATUS.off()
            count = 0
    ...
    #     if current_time - last_motion_time > MIN_MOTION_DELAY:
        if pin.value() == 1:
            ...
            if count >= max_reset_count:
                ...
                client = get_mqtt_client()
                if client:
                    client.publish('sendTelegramMessage', ujson.dumps({'message': "ALARMA ACTIVADA!!", 'name': name}))
