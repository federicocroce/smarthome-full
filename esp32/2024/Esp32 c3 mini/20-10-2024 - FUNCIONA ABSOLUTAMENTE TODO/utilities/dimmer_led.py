...
def dimmer_led(led_device, touch_dimmer_sensor, dimmer_speed):
    is_growing = led_device.brightness == 100
    flag_is_updating = False
    current_index_temperature = 0
...
#         print("callback_update_server", wifi.is_connected)
...
#                 mqtt.update_device(led_name, json_data)
                mqtt.queue_put(led_device.led_name, json_data)
#                 print("Envío al servidor", ("test", {"test": "test"}))
        else:
...
#             nonlocal duty
    ...
    while True:
        try:
            touch_dimmer_sensor.is_touching((callback_pulse, callback_two_touching, callback_three_touching), callback_update_server, callback_dimmer)
...
def set_dimmer_led(led_device, touch_dimmer_sensor_pin, dimmer_speed=10):
