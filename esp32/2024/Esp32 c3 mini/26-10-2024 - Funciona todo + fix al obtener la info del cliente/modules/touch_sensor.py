...
class Touch:
    ...
    def reset_threshold_cbk(self, callbacks_pulse, callback_touch_end, time_threshold):
        def inner_callback(pin):
            ...
            if self.touch_count == self.touch_3_max:
                ...
                if callback_pulse:
                    callback_pulse()
            if callback_touch_end:
                callback_touch_end(self)
            self.touch_count = 0
        return inner_callback

    def is_touching(self, callbacks_pulse, callback_touch_end, callback_dimmer, time_threshold=0.5):
            ...
            if touch_pin.value() == 1:
                ...
                if self.touch_type == "pulse":
                    self.timer.init(mode=Timer.ONE_SHOT,
                                    ...
                                    callback=self.reset_threshold_cbk(callbacks_pulse, callback_touch_end, time_threshold))
                    self.touch_count += 1
                else:
                    if callback_touch_end:
                        callback_touch_end(self)
