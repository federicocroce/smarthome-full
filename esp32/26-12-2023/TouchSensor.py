...
class Touch:
    ...
    def is_touching(self, callback_touch_flag_false, callback_pulse, callback_dimmer, time_threshold = 0.5):
...
#             print("self.touch_type", self.touch_type)
            if callback_pulse is not None and self.touch_type == 'pulse':
                callback_pulse()
            if callback_touch_flag_false is not None:
                delay_seconds(callback_touch_flag_false, time_threshold)
            self.touch_type = 'pulse'
