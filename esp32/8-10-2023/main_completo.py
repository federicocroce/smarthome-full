...
def dimmer_led():
    ...
    while True:
        try:
            if not cbk_dimmer:
                cbk_dimmer = device_config['led1']['device']['increase_decrease_touch'](touch_increase, touch_decrease)
                increase_decrease_delay(lambda:cbk_dimmer, 30)
...
_thread.start_new_thread(dimmer_led, ())
...
(mqtt_client, check_mqtt_connection, reconnect_mqtt) = connect_mqtt(
...
def main():
    ...
    while True:
        ...
        # print("mqtt_client.ping()", mqtt_client.ping())
        # print("mqtt_client.check_msg()", mqtt_client.wait_msg())
